...
export default function Dashboard({ onLogout, theme, toggleTheme }) {
  ...
  const handleLiveSearch = async () => {
    ...
    setIsFallback(false);
    try {
      const data = await fetchLiveSearch(liveSearchInput, selectedDate, currentUserEmail);
            ...
            <div>
                      ...
                      <div className={`bill-card ${chamber}`} onClick={() => handleBillClick(bill)}>
                        <div className="bill-card-top">
                          ...
                          <div className="bill-card-right">
                            {bill.relevance_score !== null && bill.relevance_score !== undefined && (
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.25rem', padding: '0.2rem 0.5rem', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', border: '1px solid var(--border-default)' }}>
                                <i className="fa-solid fa-brain" style={{color: 'var(--c-teal)', fontSize: '0.7rem'}}></i>
                                <span style={{fontFamily: 'var(--font-mono)', fontSize: '0.75rem', fontWeight: 'bold', color: bill.relevance_score >= 70 ? 'var(--c-teal-bright)' : bill.relevance_score >= 40 ? '#f59e0b' : '#ef4444'}}>
                                  AI Score: {bill.relevance_score}
                                </span>
                              </div>
                            )}
              ...
              <div style={{ textAlign: 'center', marginTop: '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                    <a href="/api/export/csv" className="btn btn-secondary"><i className="fa-solid fa-file-csv"></i> CSV</a>
                    <a href="/api/export/json" className="btn btn-secondary"><i className="fa-solid fa-code"></i> JSON</a>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="modal-overlay active" onClick={e => e.target.classList.contains('modal-overlay') && setSelectedBill(null)}>
          <div className="modal-container detail-modal-new">
            ...
            <div className="modal-header-new">
              <div className="modal-badges-new">
                <span className="badge-source-id">{selectedBill.source_id}</span>
                <span className="badge-stage-intro">{selectedBill.current_stage.toUpperCase()}</span>
              </div>
              <h2 className="modal-title-new">{selectedBill.title}</h2>
              <div className="modal-tags-new">
                {selectedBill.subjects.map(s => <span key={s} className="modal-tag-new">{s}</span>)}
              </div>
            </div>
            
            <div className="modal-tabs" style={{ display: 'flex', gap:'1rem', padding:'1rem 2rem 0 2rem', borderBottom:'1px solid var(--border-color)' }}>
                {['overview', 'ai', 'history'].map(t => (
                  <button 
                    key={t}
                    style={{ background:'none', border:'none', padding:'0.5rem 1rem', color: modalTab === t ? 'var(--c-teal)' : 'var(--text-secondary)', borderBottom: modalTab === t ? '2px solid var(--c-teal)' : '2px solid transparent', cursor:'pointer', fontWeight:'600', textTransform:'capitalize' }}
                    onClick={() => setModalTab(t)}
                  >{t === 'ai' ? 'AI Analysis' : t}</button>
                ))}
            </div>
              ...
              {modalTab === 'overview' && (
                <>
                  <div className="modal-section-new">
                    <h4 className="section-title-new">LEGISLATIVE PROGRESS TRACKER</h4>
                    <div className="progress-tracker-container">
                      <div className="tracker-line">
                        <div className="tracker-fill" style={{ width: `${((getProgressTracker(selectedBill).currentStep - 1) / 4) * 100}%` }}></div>
                      </div>
                      <div className="tracker-steps">
                        {getProgressTracker(selectedBill).labels.map((label, index) => {
                          const stepNum = index + 1;
                          const { currentStep } = getProgressTracker(selectedBill);
                          ...
                          return (
                            <div key={index} className={`tracker-step ${statusClass}`}>
                              <div className={`step-circle ${statusClass}`}>{stepNum}</div>
                  ...
                  <div className="modal-section-new">
                    <h4 className="section-title-new">METADATA DETAILS</h4>
                    <div className="metadata-grid-new">
                      <div className="meta-row-new"><span className="meta-key-new">Congress</span><span className="meta-value-new">{selectedBill.congress}th</span></div>
                      <div className="meta-row-new"><span className="meta-key-new">Type</span><span className="meta-value-new">{selectedBill.bill_type}</span></div>
                      <div className="meta-row-new"><span className="meta-key-new">Sponsor</span><span className="meta-value-new">{selectedBill.sponsor_name}</span></div>
                      <div className="meta-row-new"><span className="meta-key-new">Source URL</span><a href={selectedBill.source_url} target="_blank" rel="noreferrer" className="meta-value-new meta-link-new">Link</a></div>
                    </div>
                  </div>
                </>
              )}
                ...
                <div className="modal-section-new">
                  ...
                  {aiLoading ? (
                    ...
                    <div style={{padding:'2rem', textAlign:'center', background:'var(--bg-secondary)', borderRadius:'8px'}}>
                      <i className="fa-solid fa-brain" style={{fontSize:'2rem', color:'var(--c-teal)', marginBottom:'1rem'}}></i>
